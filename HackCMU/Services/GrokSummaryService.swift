...
/// Direct Grok integration using its chat-completions API.
struct GrokSummaryService: AISummaryService {
    var apiKey = GrokConfiguration.apiKey
    var endpoint = GrokConfiguration.endpoint
    var model = GrokConfiguration.model
    ...
    func weeklyDigest(_ context: DigestContext) async throws -> WeeklyDigest {
        ...
        guard !key.isEmpty, key != "PASTE_GROK_API_KEY_HERE" else {
            return try await SampleSummaryService().weeklyDigest(context)
        }
        do {
            ...
            guard (200..<300).contains(http.statusCode) else {
                ...
                case 401, 403: notice = "Grok rejected the API key or model access. Check your configuration."
                case 429: notice = "Grok's usage limit was reached. Try refreshing later."
                default: notice = "Grok is unavailable (HTTP \(http.statusCode)). Try refreshing later."
                }
            ...
            try Task.checkCancellation()
            return try await local(context, notice: "Grok could not finish this recap. Showing a local summary; refresh to retry.")
        }
    }
    ...
    static func decode(_ data: Data, context: DigestContext) throws -> WeeklyDigest {
        ...
        return WeeklyDigest(
            ...
            generatedAt: Date(), source: .live, provider: "Grok"
        )
    }
}
